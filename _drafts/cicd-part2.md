...
I need to highlight again that the idea and inspiration for this blog post series came from the videos streamed and recorded by Adrian Iliesiu aka NetGru available on the [Cisco DevNet YouTube channel](https://www.youtube.com/playlist?list=PL2k86RlAekM-Vv4pLLFYsQ6Re9BBIj4dt){:target="_blank"}. There is also the corresponding [Github repository](https://github.com/aidevnet/cicd-twitch){:target="_blank"} available.

{: style="text-align: justify" }

As I mentioned during the introduction blog post of this series there a some differences to the video series based on my own ideas. One of them will be the replacement of the Linux Shell scripts with a Python script developed by me to run the containers using Docker Compose, and doing the validation and Runner registration.

{: style="text-align: justify" }

You can follow along my step-by-step the creating of the Docker containers in this post or you can clone my GitHub repository with all files used in this blog post series. When using the CentOS 9 Linux machine we created in the first blog post then you need to install git first using ```sudo yum install git```.
...
It will install *git* with all its dependencies as you can see from the screenshot above. Then you will be able to clone my repository from GitHub. All the files used in this blog post to create the Docker containers can be found under ```./create_env/gitlab```. All other files will be used in later blog posts.
...
services:
  ...
  runner:
    container_name: runner      # add a container name
...
Now the Docker Compose file is ready and we could start running it. To make our life easier and avoid further manual tasks, I developed a small Python script. It will take care of the Docker Compose run, validate if GitLab is available, and do the final configuration including the registration of the GitLab Runner.

## Python Script to run Docker Compose

{: style="text-align: justify" }

Using Python on Whales
...
gitlab_host = 'http://192.168.11.220'
gitlab_password = '1234QWer!'
gitlab_wait_time = 25
...
def docker_compose_function():
    ...
    logging.info('Waiting for Gitlab CE to become available...')
    docker_compose = DockerClient(compose_files=['compose.yml'])
    try:
        docker_compose.compose.up(detach=True, quiet=True)
    except DockerException as e:
        logging.info(f'Exit code {e.return_code} while running {e.docker_command}')


def docker_exec(message, container, commands):
    '''Function to run Docker exec commands in containers'''
    logging.info(message)
    docker = DockerClient()
    command = commands
    try:
        docker.execute(container, command, detach=False)
    except DockerException as e:
        logging.info(f'Exit code {e.return_code} while running {e.docker_command}')
...
if __name__ == '__main__':
    ...
    # Start launching Gitlab in a thread
    logging.info('Launching Gitlab CE...')
    docker_thread = threading.Thread(target=docker_compose_function)
    docker_thread.start()

    # Check frequently if GitLab is available
    status = 503
    while status != 200:
        time.sleep(gitlab_wait_time)
        logging.info('Still waiting for Gitlab CE to become available...')
        try:
            response = requests.get(gitlab_host)
            status = response.status_code
        except:
            status = 503

    logging.info('Gitlab CE is available...')

    # Configure the external URL for GitLab using Docker exec
    message = 'Configuring external URL for GitLab...'
    commands = [
        '/bin/bash', 
        '-c',
        'echo',
        f'external_url {gitlab_host} >> /etc/gitlab/gitlab.rb'
    ]
    docker_exec(message, 'gitlab', commands)

    # Re-Configure GitLab using Docker exec
    message = 'Reconfiguring GitLab...'
    commands = ['gitlab-ctl', 'reconfigure']
    docker_exec(message, 'gitlab', commands)

    # Register the GitLab Runner to GitLab
    message = f'Registering GitLab Runner, waiting {gitlab_wait_time} second(s) for gitlab to become available...'
    time.sleep(gitlab_wait_time)
    commands = ['gitlab-runner', 'register']
    docker_exec(message, 'runner', commands)
